...
public:
  static Status Create(Font &font, int16_t base, int16_t hfu, int16_t sb, int hhu, int chu, int16_t vfu, int vhu, int lineWidth) {
    ...
    }
    {
      ...
      int16_t hwvHeight = vfu;
      Contour c;
      c.add(voleft + lineWidth, vbottom - jointLength);
      c.add(voleft, vbottom - jointLength);
      c.add(voleft, vbottom + hwvHeight);
      c.add(voright, vbottom + hwvHeight);
      c.add(voright, vbottom - jointLength);
      c.add(voright - lineWidth, vbottom - jointLength);
      c.add(voright - lineWidth, vbottom + hwvHeight - lineWidth);
      c.add(voleft + lineWidth, vbottom + hwvHeight - lineWidth);
      auto hwtobT = font.replaceSimpleGlyphByName("hwtobT", Class::Base, {c}, voright - voleft, voleft, sideBearing + hwvHeight, sideBearing);
      if (!hwtobT) {
    ...
    }
    {
      ...
      vector<Contour> contours;
      int16_t advanceHeight;
      CreateContour_O33aeL(p, voright, voright - voleft, contours, advanceHeight);
      for (auto &c : contours) {
        for (auto &point : c.points) {
          auto v = point.rotatedCW90();
          point.x = v.x;
      ...
      auto O33aoeB = font.replaceSimpleGlyphByName("O33aoeB", Class::Base, contours, w, voleft, advanceHeight, -jointLength);
      if (!O33aoeB) {
        return EGLYF_STATUS_PUSH(O33aoeB.status());
      }
    }
    for (int s = 1; s <= hhu; s++) {
