...
#include "Glyphs.hpp"
    ...
    HbBufferUniquePtr buf(CreateBuffer(A1 + vj + O1, hbFont));
    vector<GlyphInformation> infos;
    CreateGlyphInformations(buf, hbFont, infos);
    vector<string> names;
    GlyphNames(infos, *font, names);

    HbBufferUniquePtr refBuf(CreateBuffer(A9 + vj + O1, hbRefFont));
    ...
    for (size_t i = 0; i < names.size(); i++) {
      if (names[i].starts_with("A1")) {
        auto tail = names[i].substr(2);
        expect("A9" + tail == refNames[i]);
      } else {
